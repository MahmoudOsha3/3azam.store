...
<br><br>
    ...
    <header style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; flex-wrap: wrap; gap: 15px;">
        <div>
            ...
            <p class="text-muted">متابعة أداء عظمه ستور في الوقت الفعلي</p>
        </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card" style="border-right-color: #e67e22;">
            <div><h4>ارباح اليوم</h4><p id="profitOfDay">0</p></div>
            <i class="fas fa-file-invoice-dollar "></i>
        </div>

        <div class="stat-card" style="border-right-color: var(--accent);">
            <div><h4>إجمالي الطلبات علي الموقع</h4><p id="countOrders">0</p></div>
            <i class="fas fa-globe"></i>
        </div>

        <div class="stat-card" style="border-right-color: #9b59b6;">
            <div><h4>عدد طلبات هذا الشهر</h4><p id="countOrderToMonth">0</p></div>
            <i class="fas fa-user-shield"></i>
        </div>

        <div class="stat-card" style="border-right-color: var(--admin-blue);">
            <div><h4>عدد طلبات اليوم</h4><p id="countOrderToDay">0</p></div>
        ...
        <div class="stat-card" style="border-right-color: #f1c40f;">
            <div><h4>عدد المنتجات</h4><p id="ProductsCount">0</p></div>
            <i class="fas fa-box"></i>
        </div>

        <div class="stat-card" style="border-right-color: #34495e;">
            <div><h4>عدد الموظفين</h4><p id="countAdmins">0 ج.م</p></div>
            <i class="fas fa-cash-register"></i>
        </div>
    </div>
    ...
    function fetchAllData() {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                $('#countOrders').text(d.countOrders || 0);
                $('#countOrderToDay').text(d.countOrderToDay || 0);
                $('#usersCount').text(d.countUsers || 0);
                $('#countOrderToMonth').text(d.countOrderToMonth || 0);
                $('#ProductsCount').text(d.countProducts || 0);
                $('#profitOfDay').text(parseFloat(d.profitOfDay).toLocaleString() + ' ج.م');
                $('#countAdmins').text(d.countAdmins || 0);
